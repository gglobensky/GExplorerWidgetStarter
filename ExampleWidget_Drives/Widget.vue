...
const drives = ref<DriveSnapshot[]>([])
let unsubDrives: (() => void) | null = null
...
onMounted(() => {
unsubDrives = subscribeDrives(d => {
    console.log('[drives-widget] callback received:', d.length, d)
    drives.value = d
    void refreshStats()
})
})
...
<template>
  ...
  <div v-if="isToolbar" class="drives-toolbar">
    <div v-if="!drives.length" class="empty-toolbar">No drives</div>
  ...
  <div v-else class="drives-widget" :class="cfg.layout">
    <div v-if="!drives.length" class="empty">No drives found</div>
