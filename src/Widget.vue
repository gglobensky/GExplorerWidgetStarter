...
async function openEntry(FullPath: string) {
  ...
  try {
    ...
    const isDir = !!v?.isDir
    if (isDir) {
      const mode = merged.value.navigateMode
      if (mode === 'dialog') { /* unchanged */ }
      else if (mode === 'tab') { props.runAction?.({ type: 'nav', to: FullPath }) }
      else {
      // CHANGE: route folder nav to host regardless of 'internal'
      // (this unifies behavior and gets us off internal history)
        props.runAction?.({ type: 'nav', to: FullPath, sourceId: props.sourceId })
      }
      return
    }
