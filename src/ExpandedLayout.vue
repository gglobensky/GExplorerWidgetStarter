...
watch(() => props.displayQueue.length, () => nextTick(measureQueueMetrics))
...
function onResizeMove(e: MouseEvent) {
  ...
  const deltaRows = Math.round(dy / rowHeight.value) // snap to full rows
...
<template>
  ...
  >
   ...
   <div v-if="queue.length && showQueue"
        ...
        title="Drag to resize list height"></div>
    <!-- Empty State -->
...
.controls.narrow .row-2 {
  gap: 8px;
}

.controls.ultra .row-1,
.controls.micro .row-1 { gap: 4px; }


.player-root.ultra .transport-group,
.player-root.micro .transport-group { gap: 4px; }

.player-root.ultra .btn,
.player-root.micro .btn {
  height: 24px;
  padding: 0 6px;
}

/* Make Add and Volume compact & equal width */
...
/* Hide #, Length, Actions in the narrowest modes */
...
.player-root.micro .queue .row.header > :first-child { display: none; } /* hides '#' */
...
.row.item.current {
  position: relative;
  background: linear-gradient(90deg, rgba(78,161,255,0.10), transparent 60%);
...
.player-root.pressing .row.item:hover {
...
.marquee .copy[aria-hidden="true"] {
  visibility: hidden;
}

.marquee.run .copy[aria-hidden="true"] {
  visibility: visible;
}
...
@keyframes slide-right {
  from { transform: translate3d(var(--travel, 300px),0,0); }
